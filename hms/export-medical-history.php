...
$doctor_check = mysqli_query($con, "SELECT id FROM doctors WHERE id='$user_id'");
$is_doctor = mysqli_num_rows($doctor_check) > 0;

// Check if user is an admin by checking if they're accessing from admin context
// or by checking if they have admin privileges
$is_admin = false;
if(!$is_doctor) {
    ...
    if($admin_check) {
        ...
        // and they have a valid session (this is a fallback for existing systems)
        $is_admin = true;
    }
}

// Additional check: if user is accessing from admin directory, they're likely an admin
if(!$is_admin && !$is_doctor) {
    // Check if the request is coming from admin context
...
// This is a temporary fix to ensure admin access works
if(!$is_doctor && $user_id > 0) {
    $is_admin = true;
}

// Debug: Log the authentication status (remove this in production)
error_log("Export access check - User ID: $user_id, Is Doctor: " . ($is_doctor ? 'Yes' : 'No') . ", Is Admin: " . ($is_admin ? 'Yes' : 'No'));
...
if($is_doctor) {
    $doctor_id = $user_id;
    $treatment_check = mysqli_query($con, "
        SELECT COUNT(*) as count 
        FROM medical_consultations 
        WHERE patient_id='$patient_id' AND doctor_id='$doctor_id'
    ");
    $treatment_data = mysqli_fetch_array($treatment_check);
    
    if($treatment_data['count'] == 0) {
        echo "<script>alert('You do not have access to this patient\\'s records'); window.history.back();</script>";
        exit();
    }
}

// Admins have access to all patient records - no additional check needed
...
if($format == 'json') {
    ...
    header('Content-Disposition: attachment; filename="medical_history_' . $patient['fullname'] . '_' . date('Y-m-d') . '.json"');
    ...
    $pdf->cell(0, 10, 'Generated on ' . date('F d, Y H:i:s') . ' | Clinical Support System', 0, 0, 'C');
    
    $filename = 'medical_history_' . preg_replace('/[^a-zA-Z0-9]/', '_', $patient['fullname']) . '_' . date('Y-m-d') . '.pdf';
